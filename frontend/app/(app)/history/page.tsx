...
export default function HistoryPage() {
    const [activeTab, setActiveTab] = useState<"documents" | "chats">("chats");
            ...
            <header className="sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur">
                <div className="flex h-16 items-center justify-between px-6">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Your documents and conversations</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="flex bg-muted p-1 rounded-lg">
                            <button
                                onClick={() => setActiveTab("documents")}
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === "documents"
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === "chats"
                                    ? "bg-background text-foreground shadow-sm"
                                    : "text-muted-foreground hover:text-foreground"
                                    }`}
                            >
                    ...
                    chatSessions.length === 0 ? (
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                            {chatSessions.map((session) => (
                                <Link key={session.session_id} href={`/chat?doc=${session.document_ids?.[0] || 'unknown'}`}>
                                    <Card className="group hover:border-violet-500/50 transition-colors cursor-pointer h-full">
                                        <CardHeader className="pb-3">
                                            <div className="flex items-start gap-3">
                                                <div className="h-10 w-10 shrink-0 rounded-lg bg-gradient-to-br from-blue-500/20 to-cyan-500/20 flex items-center justify-center">
